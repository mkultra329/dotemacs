...
and =user-mail-address=. This is also where you want your API tokens to be.

#+BEGIN_SRC emacs-lisp
(defvar me/circe-channels         nil        "The default channels to join on connect.")
(defvar me/circe-nick             nil        "The default nick to use.")
(defvar me/circe-password         nil        "The default password to connect with.")
...
(use-package winner
  ...
  :config
  (defun me/winner-continue (&rest _)
    "Activate a sparse keymap for `winner-undo' and `winner-redo':
  <left>   `winner-undo'
  <right>  `winner-redo'"
    (set-transient-map
     (let ((map (make-sparse-keymap)))
       ...
       map)))
  (advice-add 'winner-undo :after #'me/winner-continue)
  (advice-add 'winner-redo :after #'me/winner-continue))
#+END_SRC
...
(defhydra hydra-windows (:color pink)
  (concat "\n " (me/make-hydra-heading "Windows" "Size" "Zoom" "Buffers")
          "
...
(when (eq system-type 'darwin)
  (setq-default
   ...
   ns-use-mwheel-momentum nil))                      ; Disable smoonth scroll
#+END_SRC
